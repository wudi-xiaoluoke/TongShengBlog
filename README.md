...
│       ├── static/audio/          # 首页背景音乐（mu-xin.mp3 / luo-xiao-han.mp3）
...
- **Java 集成测试**：MockMvc + MockHttpSession 直连真实库，覆盖首页随机稳定、手账按月分组、静态资源 MIME 等
- **Node 单测**：`node --test "tests/*.test.mjs"`，覆盖首页音乐首次访问/播放器逻辑、翻书纯逻辑（翻页方向、拖拽进度、跳转门控、菜单焦点决策）与零食店小游戏状态机
